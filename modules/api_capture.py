...
'''
    ...
    Copyright (C) 2012-2015  Edgar Adonai Silva Calderón
...
class triangulator(object):
    ...
    def __init__(self, resolution, distances, focal, user_height):
        '''
        Constructor
        '''
        r0 = resolution[0]/2
        ...
        self.__focal = focal
        self.__to_center = distances
        self.__rotate = [[0, 0], [1, 1], [1, 0]]
        self.__order = [[1, 0, 2], [1, 2, 0], [0, 2, 1]] 
        self.__user_height = user_height 
        return
    ...
    def __call__(self, centroids):
        ...
        return self.__user_coordinates(coordinates)
    ...
    def __retrieve(self, centroids):
        ...
        for i in xrange(3):
            for j in xrange(3):
                centroids2.append(centroids[i][0][j][0][:])
        centroids = reshape(array(centroids2), (3,3,-1)).tolist()
        viable = ((ones((3,3))*False).astype(bool).tolist())
        operation = ((ones((3))*False).astype(bool).tolist())
        print centroids
        for i_ in xrange(3):
            for j_ in xrange(3):
                if (None in centroids[i_][j_]):
                    ...
                    viable[i_][j_] = True
                    centroids[i_][j_][0] = (matrix(self.__sign_cam[i_][0]) * matrix(centroids[i_][j_][0]) + matrix(self.__center[i_][0])).tolist()
                    centroids[i_][j_][1] = (matrix(self.__sign_cam[i_][1]) * matrix(centroids[i_][j_][1]) + matrix(self.__center[i_][1])).tolist()
        print centroids
        for k_ in xrange(3):
    ...
    def __triangulate(self, centroids, focal, to_center, rotate, order):
        ...
        alpha = atan(to_center[0]/to_center[1]) - alphap
        betha = atan(to_center[1]/to_center[0]) - bethap
        miu = pi-betha-alpha
        x = L*sin(alpha)/sin(miu)*sin(bethap)
        y = L*sin(betha)/sin(miu)*sin(alphap)
        z = (centroids[0][not rotate[0]])*(to_center[0]-x)/focal[0]
        results = [int(round(x)), int(round(y)), int(round(z))]
        return [results[i] for i in order]
    
    def __user_coordinates(self, coordinates):
        '''
        '''
        coordinates = matrix(coordinates) - matrix(coordinates[0])
        dummy = coordinates
        coordinates[1] = ((dummy[1]-dummy[0])/(norm(dummy[1]-dummy[0])))*(self.__user_heigth*0.186)
        coordinates[2] = ((dummy[2]-dummy[1])/(norm(dummy[2]-dummy[1])))*(self.__user_heigth*0.146)+coordinates[1]
        return coordinates.round().astype(int).tolist()
